...
| **Build a searchable index from scratch** | `rag-literature-build` / `rhetorag-research-indexing` | build scripts |
...
for r in out["core_results"][:10]:
    print(r["doc"]["title"], "|", r.get("match_type"), "|", r["doc"].get("year"))
```
- Population/geo are **optional and query-derived**. Just type them in the query
  ("nurses", "Malaysia"); do not pass hardcoded gates.
- `match_type` is per-result: `literal` (matched your term) vs `expanded` (matched a
  query-expansion synonym). `source` is `local_catalog` or `web_fallback` (UNVERIFIED_WEB).
