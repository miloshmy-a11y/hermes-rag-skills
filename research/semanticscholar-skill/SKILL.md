...
compatibility: Requires python3 and the `requests` package. An S2 API key is already configured in the Hermes `.env` as `S2_API_KEY` (no action needed). `s2.py` reads it automatically via `os.environ["S2_API_KEY"]`. With a key, S2 grants 1 req/s across all endpoints; `s2.py` enforces a safe 1.8s gap (user-set margin). Without a key it falls back to the shared anonymous pool at a 5s gap. Invalid/expired keys (401/403) auto-drop to anonymous mode. Works unauthenticated with strict rate limits. Alternative client: the official `pip install semanticscholar` library (`from semanticscholar import SemanticScholar`) offers the same API with an SDK; `s2.py` is preferred here because it needs no extra install and matches our rate-limit/backoff conventions.
...
- **Key location:** Hermes `.env` → `S2_API_KEY=...` (already set; `s2.py` reads it via `os.environ`). Do NOT hardcode the key in scripts or skill files.
- **Limit:** 1 request/second across ALL endpoints (cumulative). `s2.py` enforces a **1.8s gap** with a key (safe margin under 1 req/s), 5s without.
- **Backoff:** automatic exponential backoff on 429/504 (2s→60s, 5 retries). 401/403 on a key → auto-drop to anonymous mode and retry.
- **Auth header:** `x-api-key: <S2_API_KEY>` (per S2 tutorial). Already applied in `s2.py`.
- **Tip:** use batch/bulk endpoints (`batch_papers`, `search_bulk`) to minimize request count.
