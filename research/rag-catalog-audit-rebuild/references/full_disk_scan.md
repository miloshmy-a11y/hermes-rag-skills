...
```
PDFTO="$(cygpath -w "$(command -v pdftotext)")"   # portable: resolve pdftotext from PATH (Git-bash/MSYS); on Windows use the absolute path from `cygpath -w /mingw64/bin/pdftotext`
