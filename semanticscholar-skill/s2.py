...
# Per S2 docs: API keys get an introductory 1 req/s dedicated quota across
# all endpoints (cumulative). We stay safely UNDER that with a 1.8s gap (user
# instruction: keep below 1 req/s with margin to avoid 429s). Without a key,
# anonymous calls share a global pool that vanishes under load, so we use 5s.
_last_request_time = 0
_MIN_GAP = 1.8 if HAS_KEY else 5.0
...
def _request(method, url, params=None, json_data=None, max_retries=5):
    ...
    for attempt in range(max_retries + 1):
        ...
        if r.status_code in (401, 403) and HAS_KEY:
            # Invalid/expired key (S2 returns 401 Unauthorized or 403 Forbidden
            # for bad credentials per the API tutorial). Drop the key and retry
            # unauthenticated this attempt rather than looping on a dead key.
            import s2 as _self
            print(f"  [auth] S2_API_KEY rejected ({r.status_code}); switching to unauthenticated mode", file=sys.stderr)
            _self.HAS_KEY = False
            _self.HEADERS = {}
            _self._MIN_GAP = 5.0
            continue
